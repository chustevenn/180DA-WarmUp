...
while True:
    ...
    x_threshold = abs(0.5*prevXAcc)
    z_threshold = abs(0.2*prevZAcc)
    x_accel = (magXcomp - prevXAcc > x_threshold)
    z_accel = (MAGz - prevZAcc > z_threshold)
    prevXAcc = magXcomp
    prevZAcc = MAGz

    if x_accel:
        print('X acceleration = %5.2f ' % magXcomp + 'FORWARD PUSH\n')

    if z_accel:
        print('Z acceleration = %5.2f ' % MAGz + 'UPWARD LIFT\n')
